...
func Connect2_UploadMedicalInformation(user *model.User_solidity,uid int) error {
	cliadd :=db.Get()
	var pk string
	cliadd.Select("select private_key from user where id = ? ",uid).Find(&pk)
	nonce, err := eth.Client.PendingNonceAt(context.Background(), common.HexToAddress(pk))
	if err != nil {
		log.Fatal(err)
		return err
	}
	...
	clipk :=db.Get()
	var add string
	clipk.Select("select address from user where id = ? ",uid).Find(&add)
