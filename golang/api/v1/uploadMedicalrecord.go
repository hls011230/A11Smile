...
func ReleaseMedicalInformation(gid int,gainer *model.Soliciter_solidity) error {
	...
	_, err = eth.Ins.GainerAddMedicalInformation(auth,big.NewInt(gainer.Min_),big.NewInt(gainer.Max),big.NewInt(gainer.Account),gainer.Department,gainer.MedicalName,gainer.MedicalNeed,gainer.RequirementDescription)
	if err != nil {
		log.Fatal(err)
		return err
	}
