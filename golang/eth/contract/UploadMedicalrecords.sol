...
contract UploadMedicalrecords is ERC20{
    ...
    //用户上传体检报告
    struct user_MedicalExaminationReportstrucr{
        address user;
        string[] PictureRoute;
    }
    ...
    //mapping用户对应上传体检报告
    mapping (address => user_MedicalExaminationReportstrucr) private userAMedicalExaminationReport;
    ...
    //用户上传体检报告
    function user_MedicalExaminationReport(string memory Proute) public {
        require(user_IsUser(msg.sender),"User no exist");
        userAMedicalExaminationReport[msg.sender].user=msg.sender;
        userAMedicalExaminationReport[msg.sender].PictureRoute.push(Proute);
    }
